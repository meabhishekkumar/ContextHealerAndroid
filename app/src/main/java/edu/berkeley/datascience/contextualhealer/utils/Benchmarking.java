...
public class Benchmarking {
    ...
    public void CheckFeaturEngineering(){
        List<double[]> inputs = readCsv(mContext, "raw_input.csv");
        //Log.v(TAG, " row 1 : " + Arrays.toString(inputs.get(0)));

        PredictionSample sample = new PredictionSample();
        for(double[] input: inputs){
            sample.AddAccelerometerX(input[0]);
            sample.AddAccelerometerY(input[1]);
            sample.AddAccelerometerZ(input[2]);
        }

        double[] features = sample.GetSample2();
        Log.v(TAG, "Sample Count" + sample.Count() + " Features : " + Arrays.toString(features));
    }
